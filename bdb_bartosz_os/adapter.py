...
class AdapterOperator(Protocol):
    ...
    def sessions(self, workspace_root: str, **kwargs: Any) -> OperatorResponse: ...
...
class BartoszOsAdapter:
    ...
    def _dispatch(self, request: BartoszOsRequest) -> OperatorResponse:
        ...
        if operation == "sessions":
            _require_keys(params, {"workspace_root"}, {"limit"})
            root = _path(params, "workspace_root")
            kwargs = {"limit": params["limit"]} if "limit" in params else {}
            return self._operator.sessions(root, **kwargs)
        if operation == "logs":
