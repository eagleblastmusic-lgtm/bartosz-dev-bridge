...
def _build_execution_prompt(project: ProjectRecord, heading: str, *, plan: ProjectPlan | None = None, state: ProjectMemoryState | None = None, git_head: str | None = None, binding: ProjectExecutionBinding | None = None) -> str:
    ...
    lines = [
        ...
        f"Plan version (JSON string): \"{current_plan.plan_version if current_plan else project.plan_version}\"",
        ...
        "Jeśli czekasz na aktywne zewnętrzne CI/build, opisz WAITING_EXTERNAL i odnośnik w zwykłej aktualizacji; nie twórz jeszcze finalnego JSON.",
        ...
        "Dopiero po zakończeniu wykonania i walidacji zwróć dokładnie jeden blok JSON (Nie YAML i bez dodatkowych BDB_SUBMISSION bloków) zgodny ze schematem bdb-project-execution-submission-v1. WAITING_EXTERNAL/PENDING/RUNNING/VALIDATING/AWAITING_CI nie są finalnym wynikiem.",
        "Skopiuj project_id, task_id, execution_binding_id, correlation_id, command_id, repo_alias i head_before dokładnie z tego promptu/bindingu; plan_version musi być tekstem JSON w cudzysłowie, np. \"1\".",
