...
async function projectLaunchAck(launchId, claimId, conversationId = null, handoff = null) {
  let explicitConversation = null;
  let handoffObj = handoff;
  if (typeof conversationId === "string") {
    explicitConversation = boundedId(conversationId, "conversation_id");
  } else if (conversationId && typeof conversationId === "object" && handoff === null) {
    handoffObj = conversationId;
  }
  const metadata = handoffObj && typeof handoffObj === "object" ? {
    handoff_status: "SENT",
    project_id: boundedId(handoffObj.project_id, "project_id"),
    execution_binding_id: boundedId(handoffObj.execution_binding_id, "execution_binding_id"),
    conversation_id: boundedId(handoffObj.conversation_id, "conversation_id")
  } : {};
  const finalConversation = metadata.conversation_id || explicitConversation;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  const operation = message.type === "bdb-vnext-status" ? status()
    ...
    : message.type === "bdb-vnext-project-launch-ack" ? projectLaunchAck(message.launch_id, message.claim_id, message.conversation_id, message.handoff)
