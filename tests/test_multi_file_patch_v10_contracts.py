...
def test_v10_registry_name_statements_and_literal_checksum() -> None:
    ...
    assert MIGRATION_V10.checksum() == V10_CHECKSUM


def test_v1_through_v9_checksums_remain_frozen() -> None:
    assert tuple(migration.checksum() for migration in MIGRATIONS[:9]) == (
