...
def test_service_journal_flow(tmp_path: Path) -> None:
    ...
    assert inst1.instance_id == "inst-1"
    ...
    assert active.instance_id == "inst-1"

    latest = journal.get_latest_service_instance()
    assert latest is not None
    assert latest.instance_id == "inst-1"
    ...
    journal.heartbeat_service_instance("inst-1")
    inst1_hb = journal.get_service_instance("inst-1")
    assert inst1_hb is not None
    assert inst1_hb.heartbeat_at == FIXED_NOW

    # 5. Heartbeat for non-existent fails
    with pytest.raises(BridgeError) as exc:
        journal.heartbeat_service_instance("inst-nonexistent")
    assert exc.value.code == BridgeErrorCode.JOURNAL_CONFLICT

    # 6. Request stop transitions running -> stopping
    stop_outcome = journal.request_service_stop("inst-1")
    assert stop_outcome.instance_id == "inst-1"
    assert stop_outcome.status == ServiceStatus.STOPPING
    assert stop_outcome.stop_requested is True

    inst1_stopping = journal.get_service_instance("inst-1")
    ...
    stop_outcome_again = journal.request_service_stop("inst-1")
    ...
    inst1_stopped = journal.get_service_instance("inst-1")
    ...
    assert inst2.instance_id == "inst-2"
    assert journal.get_active_service_instance().instance_id == "inst-2"

    # Mark failed
    journal.mark_service_instance_failed("inst-2", error="forced error")
    inst2_failed = journal.get_service_instance("inst-2")
    ...
    # Start inst-3
    journal.start_service_instance("inst-3", pid=789, started_at="2026-07-15T12:10:00Z")
    assert journal.get_active_service_instance().instance_id == "inst-3"
    
    stale_count = journal.mark_abandoned_service_instances_stale("process crashed")
    assert stale_count == 1
    inst3_stale = journal.get_service_instance("inst-3")
    ...
    events = journal._connection.execute(
