...
def test_empty_db_applies_all_migrations(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert migrations == [
            ...
            (9, "journal_v9_multi_file_patch_recovery"),
            (10, "journal_v10_multi_file_patch_runtime"),
            (11, "journal_v11_shared_direct_checkout_paths"),
            (12, "journal_v12_staged_validation"),
        ]
    finally:
        journal.close()
...
def test_future_schema_version_rejected(tmp_path: Path) -> None:
    ...
    journal._connection.execute(
        "INSERT INTO schema_migrations (version, name, checksum, applied_at) VALUES (?, 'future', 'abc', ?)",
