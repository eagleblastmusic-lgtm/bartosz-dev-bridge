...
def test_v7_registry_and_literal_checksum() -> None:
    assert [m.version for m in MIGRATIONS] == list(range(1, 12))
...
def test_v6_upgrade_preserves_data(tmp_path: Path, populated: bool) -> None:
    ...
    journal = Journal.open(path, now_fn=lambda: NOW)
    assert journal._connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0] == 11
    if populated:
...
def test_future_version_rejected_after_v8(tmp_path: Path) -> None:
    ...
    journal._connection.execute(
        "INSERT INTO schema_migrations(version,name,checksum,applied_at) VALUES(12,'future','x',?)",
        (NOW,),
    )
