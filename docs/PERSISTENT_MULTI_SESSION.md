...
- exactly one Bridge service instance is `running` and none is `stopping`;
...
2. If the active session has no claimable command, the handoff preconditions are checked in one journal transaction.
